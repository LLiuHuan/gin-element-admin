...
func RateLimitMiddleware(fillInterval time.Duration) gin.HandlerFunc {
	bucket := ratelimit.NewBucketWithQuantum(fillInterval, global.GEA_CONFIG.RateLimit.Cap, global.GEA_CONFIG.RateLimit.Quantum)
...
func IpVerifyMiddleware() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		// 判断是否在黑名单
		timeOrigin := global.GEA_REDIS.HGet(global.GEA_CONFIG.RateLimit.IpListKey, visitorIP).Val()
		err := blackListVerify(timeOrigin, visitorIP, global.GEA_REDIS, blacklist[visitorIP])
		if err != nil {
		...
		// 如果不存在于黑名单
		lenList, _ := global.GEA_REDIS.LLen(visitorIP).Result()
		// 如果第一次登陆或频率限制外
		if lenList == 0 {
			// 如果为空跳过，设置后跳过
			// 添加IP list
			global.GEA_REDIS.LPush(visitorIP, visitorIP)
			// 设置过期时间
			global.GEA_REDIS.Expire(visitorIP, time.Second)
			c.Next()
		} else if lenList > 0 && lenList < global.GEA_CONFIG.RateLimit.IpLimitCon {
			global.GEA_REDIS.LPush(visitorIP, visitorIP)
			c.Next()
		} else {
			...
			// 加入黑名单
			global.GEA_REDIS.HSet(global.GEA_CONFIG.RateLimit.IpListKey, visitorIP, time.Now().Local().Unix())
			c.Abort()
			return
		}
	}
...
func blackListVerify(ot, visitorIP string, rdb *redis.Client, limitTime int64) error {
	...
	if ot != "" {
		...
		if subTime > time.Duration(limitTime)*time.Minute {
			// 超过限制时间 解除限制
			rdb.HDel(global.GEA_CONFIG.RateLimit.IpListKey, visitorIP)
			return nil
		} else {
